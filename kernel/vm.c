...
pagetable_t kkvminit() {
  ...
  kkvmmap(kpagetable, KERNBASE, KERNBASE, (uint64)etext - KERNBASE,
          PTE_R | PTE_X);
  kkvmmap(kpagetable, (uint64)etext, (uint64)etext, PHYSTOP - (uint64)etext,
          PTE_R | PTE_W);
...
void kkvmmap(pagetable_t kpagetable, uint64 va, uint64 pa, uint64 sz,
             int perm) {
...
// used in fork, copy new content in process's page table to its kernel
// pagetable
void u2kvmcopy(pagetable_t upagetable, pagetable_t kpagetable, uint64 oldsz,
               uint64 newsz) {
  oldsz = PGROUNDUP(oldsz);
